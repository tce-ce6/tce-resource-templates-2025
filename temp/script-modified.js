document.addEventListener("DOMContentLoaded", function () {
  ...
  function adjustLayout(data, targetImgElement, svg) {
    ...
    data.containers.forEach((containerData) => {
        let boxWidth = containerData.width;
        let boxHeight = containerData.height;

        // Ensure box positions are within screen bounds
        let boxLeft = Math.max(0, Math.min(containerData.boxPosition.left, SCREEN_WIDTH - boxWidth));
        let boxTop = Math.max(0, Math.min(containerData.boxPosition.top, SCREEN_HEIGHT - boxHeight));

        let boxBorderColor = containerData.borderColor;
        let boxBackgroundColor = containerData.backgroundColor;
